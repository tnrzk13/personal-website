import type { ProjectData } from "../types";

export const projList: ProjectData[] = [
  {
    title: "Pneumonia Detection",
    imgBase: "images/04-project/pneumoniaXrayIdentifier",
    urls: {
      projectUrl: "https://huggingface.co/spaces/tnrzk13/PneumoniaDetection",
      codeUrl: "https://github.com/tnrzk13/PneumoniaXrayIdentifier",
    },
    text: "Deep Learning model that diagnoses pneumonia with <strong>98.5% accuracy</strong> from chest X-ray images, built with FastAI and deployed on HuggingFace.",
    details: [
      "Fine-tuned a pretrained CNN on <strong>~5,800 X-ray images</strong> - identified and removed corrupt samples that were degrading accuracy",
      "Achieved <strong><3% error rate</strong> validated through confusion matrix and top-loss analysis to understand where the model struggles",
      "Shipped a <strong>live Gradio interface</strong> on HuggingFace Spaces - anyone can upload an X-ray and get a diagnosis in seconds",
    ],
    techstack: ["Python", "Fast.ai", "Gradio"],
    tags: ["ML"],
    tier: "featured",
    oneliner: "Deep learning X-ray diagnosis model",
  },
  {
    title: "Voice Dictation",
    imgBase: "images/04-project/voicedictation",
    videoUrl: "images/04-project/voicedictation.mp4",
    urls: {
      projectUrl: "",
      codeUrl: "https://github.com/tnrzk13/voice-dictation",
    },
    text: "Real-time Linux voice transcription tool with a <strong>client-daemon architecture</strong> that keeps Whisper loaded for minimal latency, featuring streaming self-correction and spoken formatting commands.",
    details: [
      "Designed a <strong>persistent daemon</strong> that keeps Whisper loaded in GPU memory - eliminates the cold-start penalty so dictation feels instant",
      "Streams raw PCM over a <strong>Unix socket</strong> and uses diff-based typing so words appear and self-correct in real time",
      "Built a <strong>spoken command system</strong> for punctuation and symbols with deduplication against Whisper's own auto-punctuation",
    ],
    techstack: ["Python", "Whisper", "CUDA"],
    tags: ["Systems"],
    tier: "featured",
    oneliner: "Real-time Linux voice transcription tool",
  },
  {
    title: "YouTube Audio Chunker",
    imgBase: "images/04-project/youtubechunker",
    urls: {
      projectUrl: "",
      codeUrl: "https://github.com/tnrzk13/youtube-audio-chunker",
    },
    text: "<strong>Tauri desktop app</strong> that downloads YouTube audio, splits it into navigable chunks, and sideloads to Garmin watches, with a SvelteKit frontend and Python processing backend.",
    details: [
      "<strong>Full-stack desktop app</strong> - SvelteKit GUI, Tauri/Rust shell, and Python CLI working together as a sidecar architecture",
      "End-to-end pipeline: yt-dlp download, <strong>lossless ffmpeg splitting</strong>, and ID3 tagging for sequential playback on the watch",
      "Handles <strong>device sync via MTP</strong> with status tracking and automatic re-sync when metadata changes",
    ],
    techstack: ["Python", "SvelteKit", "Tauri", "ffmpeg"],
    tags: ["Web"],
    tier: "featured",
    oneliner: "Desktop app for Garmin audio sideloading",
  },
  {
    title: "SoulDog",
    imgBase: "images/04-project/souldog",
    urls: {
      projectUrl: "https://souldog.herokuapp.com",
      codeUrl: "https://github.com/aarshio/SoulDog-CS348",
    },
    text: 'Awarded <strong>"top project of the class"</strong> in CS348: Database Systems. Full-stack webapp matching abandoned dogs with new owners, featuring Google auth, search, and posting.',
    techstack: ["Javascript", "React", "Node.JS", "Knex JS", "SQL"],
    tags: ["Web"],
    tier: "compact",
    oneliner: "Full-stack dog adoption webapp",
  },
  {
    title: "Time Series Forecasting",
    imgBase: "images/04-project/airpassengers",
    urls: {
      projectUrl: "",
      codeUrl: "https://github.com/tnrzk13/TimeSeriesAirPassengers",
    },
    text: "Forecasted air passenger volumes <strong>36 months ahead</strong> using SARIMA and Holt-Winters Exponential Smoothing models.",
    techstack: ["Python", "SciKit Learn", "StatsModels", "Matplotlib", "Pandas", "SciPy"],
    tags: ["ML"],
    tier: "compact",
    oneliner: "Air passenger volume prediction",
  },
  {
    title: "This Website!",
    imgBase: "images/04-project/website",
    videoUrl: "images/04-project/website.mp4",
    urls: {
      projectUrl: "https://tnrzk13.github.io/personal-website/",
      codeUrl: "https://github.com/tnrzk13/personal-website",
    },
    text: "Built with <strong>Svelte</strong> from scratch - learned a new framework, brushed up on HTML and CSS, and solved lots of bugs along the way.",
    techstack: ["Javascript", "Svelte"],
    tags: ["Web"],
    tier: "compact",
    oneliner: "Svelte portfolio site built from scratch",
  },
  {
    title: "Chess",
    imgBase: "images/04-project/chess",
    urls: {
      projectUrl: "",
      codeUrl: "https://gitfront.io/r/user-1077305/ELvfgXx6E716/chess/",
    },
    text: "Full chess implementation with <strong>4 difficulty levels</strong> including Stockfish, supporting human-vs-human and human-vs-CPU. Built with Observer and Factory Method patterns.",
    techstack: ["C++", "Makefile"],
    tags: ["Systems"],
    tier: "compact",
    oneliner: "Full chess engine with Stockfish AI",
  },
  {
    title: "Wumpus World",
    imgBase: "images/04-project/wumpusworld",
    urls: {
      projectUrl: "",
      codeUrl: "https://gitfront.io/r/user-1077305/epuyjAbpQqjR/wumpusworld/",
    },
    text: "Solved an RPG-like problem using <strong>reinforcement learning</strong> (Q-Learning, SARSA) paired with exploration strategies (greedy, softmax) to find optimal combinations.",
    techstack: ["Python"],
    tags: ["ML"],
    tier: "compact",
    oneliner: "Reinforcement learning RPG solver",
  },
];
